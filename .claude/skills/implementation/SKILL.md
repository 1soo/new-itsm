...
| FE 또는 BE | Next가 지정된 경우(CSR: Frontend 역할 / SSR: 서버 역할까지 겸함) | `references/next/` |
| BE | Backend가 Spring/Spring Boot인 경우 | `references/spring-boot/` |
| DB | 모든 경우(DB 구성) | `references/database/` |
| DB | `tech.md`에서 Database/BaaS로 **Supabase**가 지정된 경우(인증·DB·File Storage·Realtime·자동 생성 API 중 요구사항에 필요한 기능만 선택적으로 사용) | `references/database/supabase/` |

각 폴더에는 구현 규칙인 `conventions.md`와 MVC·SOLID·GoF·DDD 등 설계·디자인 패턴 상세인 `patterns.md`가 함께 있다. `conventions.md`를 먼저 참조하고, 패턴의 구체적 적용법이 필요할 때 같은 폴더의 `patterns.md`를 참조한다(`conventions.md`의 "핵심 규칙"에도 `patterns.md` 경로가 링크되어 있다).
