...
- 요구사항/기능 명세서에 명시된 화면과 요소만 설계한다. 추측·확장은 금지한다.
- **색상**: 요구사항에 적절한 base 색상을 먼저 선택하고, 화면 요소의 색상은 **그 base 색상에서 크게 벗어나지 않는 범위로 variation**하여 구성한다. (일관된 톤 유지)
- 각 화면은 연관된 요구사항(REQ-ID)/기능(FEAT-ID)과 연결한다.
