...
1. `dev-lead`로부터 전달받은 도메인별 수정 내역을 확인한다.
2. 도메인별로 `docs/06_maintenance/{yyyyMMdd-HHmmss}/{domain}/report.md`를 template에 맞춰 작성한다. (`{yyyyMMdd-HHmmss}` = 유지보수 완료 보고를 전달받은 시각, 도메인이 여러 개여도 같은 실행 회차는 동일한 타임스탬프를 공유한다)
