...
`dev-lead`가 전달한 유지보수 개발 완료 내역을 도메인별로 정리해 이력 리포트를 작성하고, 인덱스를 갱신한 뒤 직접 커밋·push한다.
...
4. `Bash`로 `git add docs/06_maintenance` → `git commit` → `git push origin main`을 직접 수행한다. (다른 경로는 add 대상에 포함하지 않는다 — 도메인 소스코드는 이미 `dev-lead`가 커밋 완료한 상태)
...
- git 커밋·push: `maintainer`가 직접 수행
