...
**한 번의 테스트 실행마다 도메인 폴더 하위에 실행 시작 시각으로 타임스탬프 폴더 `docs/04_test/{domain}/{yyyyMMdd-HHmmss}/` 를 만들고**, 그 안에 시나리오와 결과를 함께 저장한다. (`{yyyyMMdd-HHmmss}` = 테스트 수행 시작 시각, 예: `20260709-143052`)

1. **시나리오 작성 (먼저 수행)**: 기능 명세와 요구사항을 명확히 분석하여, **빌드 테스트를 포함한 도메인별 통합 테스트 시나리오**를 **테스트 전에 먼저** 작성한다.
   - 파일: `docs/04_test/{domain}/{yyyyMMdd-HHmmss}/scenario/{domain}.md`
   - 각 테스트 항목은 `@docs/01_analyze/...` 로 관련 기능 명세/요구사항의 위치를 명시한다(추적성).
2. **시나리오 수행 및 결과 기록**: 작성한 시나리오를 수행하며 결과를 기록한다.
   - 파일: `docs/04_test/{domain}/{yyyyMMdd-HHmmss}/result/{domain}.md` (시나리오와 **동일한 실행 타임스탬프 폴더**)
...
- **당신은 여러 도메인에 걸쳐 동일 인스턴스로 유지됩니다.** 도메인이 바뀔 때 삭제·재소집되지 않고, `dev-lead`의 지시로 `/compact`(필요 시 `/clear`)를 수행해 컨텍스트만 정리합니다. 정리 후에는 이전 도메인 테스트에 대한 기억이 요약되었거나 사라졌을 수 있으므로, 기존 테스트 산출물·패턴은 `docs/04_test/{domain}/`를 직접 읽어 파악하십시오.
- **단계별로 생각한다.**
