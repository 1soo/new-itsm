...
4. **이력 전달**: 모든 도메인의 유지보수 개발+테스트가 완료되면 `dev-lead`가 도메인별 수정 내역을 `maintainer`에게 전달한다.
5. **이력 생성 (`maintainer`)**: `maintenance-history-report` skill로 `docs/06_maintenance/{yyyyMMdd-HHmmss}/{domain}/report.md`에 이력을 저장하고, `docs/06_maintenance/CLAUDE.md` 인덱스를 갱신한다.
