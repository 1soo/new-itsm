...
| 유지보수  | `maintainer`           | 유지보수 요청 분석(이력 조회 후 designer 전달), 유지보수 이력 기록 및 커밋·push                                                                                                              |
...
6. **이력 생성 (`maintainer`)**: `maintenance` skill(`references/history-report`)로 `docs/06_maintenance/{yyyyMMdd-HHmmss}/{domain}/history.md`에 이력을 저장하고 `docs/06_maintenance/CLAUDE.md` 인덱스를 갱신한다. (`{yyyyMMdd-HHmmss}`는 `dev-lead`가 전달한 타임스탬프를 그대로 사용, 임의 생성·추측 금지)
7. **이력 커밋**: 이력 생성 후 `maintainer`가 직접 이력 문서(`docs/06_maintenance/` 변경분)를 `git add` → `commit` → `push`(main)하고, 완료되면 `dev-lead`에게 알린다.
...
- **커밋 주체·시점**: `dev-lead`가 **도메인 하나의 개발+테스트 완료 때마다** `git add` → `commit` → `push`(main)한다. 유지보수 워크플로우에서 `maintainer`가 생성하는 이력 문서(`docs/06_maintenance/`)는 `maintainer`가 직접 `git add` → `commit` → `push`(main)한다.
