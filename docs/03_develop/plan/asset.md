...
## 승인 대상자 역할 기반 동적 상세조회 권한 — ASSET 부분 (유지보수 요청, 2026-07-15)

> 8개 도메인 공용 작업. 전체 설계·담당범위·완료기준은 `docs/03_develop/plan/common.md` 동일 제목 절 참조. **결론: 스킵(코드 변경 없음)** — 확인 결과 `asset/application/AssetService.java` `detail()`의 의도된 RBAC는 원래부터 "인증된 사용자 전반 허용"이었다(ASSET_MANAGER 전용이 아님, 코드 주석 근거). `approver.md` v0.3이 ASSET을 다른 4개 도메인과 같은 "매니저 전용" 묶음으로 잘못 서술했던 설계 문서 오류로, designer가 2026-07-15 정정 완료(`docs/02_plan/api_spec/common.md` 0-1절, `docs/02_plan/security/authorization/approver.md`). APPROVER는 이미 전체조회 가능해 canApproverView 추가가 무의미하므로 이번 유지보수에서 ASSET 상세조회 코드는 그대로 둔다. FE 라우트 가드(`routes/index.tsx`)에 ROLE_APPROVER를 추가한 것은 그대로 유효(추가해도 무해 — 이미 전체 공개라 실질적 영향 없음).

## 전이 버튼 라벨 (유지보수 요청, 2026-07-16)

### 설계 근거
- 화면: `docs/02_plan/screen/asset.md` v0.4 SCR-ITAM-003(생애주기 단계 전이 라벨표, 85행 이하). "폐기" 버튼은 도착 상태명과 동작 동사형이 동일해 변경 없음.
- 공통 아키텍처: `docs/03_develop/plan/common.md` "상태 전이 버튼 라벨·타임라인 actor 공통 아키텍처" 절. **ASSET은 타임라인 actor 대상 아님**(BE/DB 변경 없음, FE 단독).

### 담당 범위 — dev-fe 단독(BE/DB 없음)
- `features/asset/status.ts`에 `transitionLabel(t, target: AssetStatus): string` 신규(i18n 키 `asset:transition.*`, 매핑값은 SCR-ITAM-003 표. "폐기" 단계는 기존 라벨과 동일하므로 매핑값도 동일하게 둔다). 기존 `statusLabel`은 변경하지 않음.
- `AssetDetailPage.tsx`의 생애주기 단계 전이 버튼 텍스트만 `statusLabel(t, target)` → `transitionLabel(t, target)`으로 교체(폐기 버튼 텍스트는 결과적으로 동일한 값이라 그대로 두어도 되나, 일관성을 위해 동일하게 `transitionLabel` 사용 권장). 전이 완료 토스트는 `statusLabel` 유지.

### 완료 기준
- 자산 상세(SCR-ITAM-003)의 생애주기 전이 버튼에 동작 동사형 라벨이 표시되고, 전이 완료 토스트는 기존처럼 도착 상태명을 사용한다.
