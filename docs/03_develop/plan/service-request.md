...
## 개발 계획 — 2026-07-18 유지보수 6차: 개별 미리보기 제거 + 라벨 표시 보정

- 요구사항 4건(직전 2차/5차 결과에 대한 정리·결함 수정):
  1. Content 설정 팝업 내부의 컴포넌트별 개별 실시간 미리보기(2차 신규분, 상호작용 가능한 것)를 완전히 제거한다. 캔버스=미리보기 통합(카드 자체 렌더링)과 SCR-SRM-007 축소 미리보기(A1)는 그대로 유지 — 실제 렌더링 확인은 이 둘로 충분하다고 판단해 중복 기능 정리.
  2. 라벨 텍스트를 오버레이 안쪽 좌측 상단이 아니라 `fieldset`/`legend`처럼 **테두리 선 위에 걸치는 스타일**로 변경(텍스트를 테두리 선 기준 텍스트 높이 절반만큼 위로 올리고, 불투명 배경으로 그 아래 테두리 선을 가림).
  3. **결함 수정**: 오버레이 렌더링 여부(참조 컴포넌트 1개 이상이면 항상 렌더링)와 테두리 스타일 적용 여부(`showBorder`로만 제어)를 분리 — 직전 5차 구현이 `showBorder=false`일 때 오버레이 `<div>` 전체를 안 그려서 텍스트까지 함께 사라지던 결함이 있었다. 이제 오버레이는 참조 1개 이상이면 항상 렌더링하고, 테두리 관련 클래스/스타일만 `showBorder`로 조건부 적용, 텍스트는 `showBorder`와 무관하게 항상 표시.
  4. 라벨 생성/수정 미니 팝업에서 "테두리 없음" 체크박스 상태에 따라 테두리색(`borderColor`) 입력 UI 자체를 조건부 렌더링(체크 시 숨김, 체크 해제 시 다시 표시) — `borderColor` 값 자체는 숨겨져 있어도 폼 상태에 보존(체크 해제 시 이전 값 그대로 복귀).
- 설계 근거: `docs/02_plan/screen/service-request.md` 5.4절(개별 미리보기 제거, 팝업 크기 문단 갱신)/5.8절(생성 팝업 조건부 렌더링, 오버레이 조건 분리, legend 스타일 텍스트). API/DB 데이터 모델 변경 없음(순수 FE UI 조정) — dev-be/dev-db 소집 불필요.
- 참고 기존 코드: `source/frontend/src/components/common/dynamic-form-builder.tsx`(`ComponentSettingsPopover`의 "미리보기" 섹션, 라벨 생성 팝업, 캔버스 경계 오버레이 — 전부 이전 차수에서 이미 구현됨).

### 담당 범위

#### FE (dev-ui) — `source/frontend/src/components/common/dynamic-form-builder.tsx`

1. `ComponentSettingsPopover` 최하단의 "미리보기" 섹션(`GridComponentBody`+`previewValue` state 사용부) 전체 삭제. 관련 로컬 state(`previewValue`)도 함께 제거. `GridComponentBody` import는 캔버스 카드 렌더링(`BuilderComponentCard`)에서 여전히 쓰이므로 유지(개별 미리보기 쪽 사용부만 제거).
2. 라벨 생성/수정 미니 팝업: 테두리색 `<input type="color">` 블록을 `labelForm.showBorder`가 `false`면 렌더링하지 않도록 조건부 처리(체크박스와 같은 폼 안, 순서는 FE 재량). 값은 계속 `labelForm.borderColor`에 보존(체크박스 토글만으로는 이 값을 지우지 않음 — 이미 5차에서 그렇게 구현돼 있으므로 확인만).
3. 캔버스 경계 오버레이(`labels.map(...)` 블록): 현재 `refs.length < 1 || label.showBorder === false`로 스킵하던 조건을 `refs.length < 1`(참조 0개일 때만 스킵)로 변경 — `showBorder` 값과 무관하게 참조 1개 이상이면 오버레이 자체는 항상 렌더링. 대신 오버레이 `<div>`의 테두리 관련 클래스(`border-2`, `borderColor` 스타일)만 `label.showBorder !== false`일 때만 적용(false면 테두리 관련 클래스/스타일 생략, 배경·포지셔닝 등 나머지는 유지).
4. 라벨 텍스트 표시를 legend 스타일로 변경: 현재 오버레이 내부 좌측 상단 `absolute left-0 top-0` 텍스트를, 테두리 선 위에 걸치도록 위치 조정(텍스트를 테두리 선 기준 절반 높이만큼 위로 이동 — 예: `-top-[Npx]`류 음수 오프셋, N은 텍스트 높이의 절반에 맞춰 FE 재량 산정) + 불투명 배경(`bg-background` 계열, 반투명이 아니라 아래 선을 가릴 정도의 불투명도)을 유지해 그 아래 지나는 테두리 선을 가린다. `showBorder=false`로 테두리 자체가 없을 때도 텍스트는 (테두리가 있었다면 걸쳐 있었을) 동일한 위치에 표시한다.

### 진행 순서

1. 단일 파일(`dynamic-form-builder.tsx`) 내 독립적인 3개 변경(개별 미리보기 제거·라벨 생성 팝업 조건부 렌더링·캔버스 오버레이 조건 분리+legend 스타일)이라 순서 무관, 한 번에 진행 가능.

### 완료(테스트 통과) 기준

- Content 설정 팝업에 더 이상 "미리보기" 섹션이 없음(입력 위젯만 존재). 캔버스 카드 자체 렌더링과 SCR-SRM-007 축소 미리보기(A1)는 그대로 정상 동작.
- 라벨 생성/수정 팝업에서 "테두리 없음" 체크 시 테두리색 입력 UI가 사라지고, 체크 해제 시 이전에 입력했던 색상 값 그대로 다시 나타남(값이 초기화되지 않음).
- `showBorder=false`인 라벨이 컴포넌트 1개 이상에 지정되면, 캔버스에 테두리 선은 안 보이지만 라벨 텍스트는 여전히 표시됨(직전 5차 결함 수정 확인). 참조 0개면 오버레이 자체가 없음(텍스트도 없음).
