...
### 설계 근거
- 화면: `docs/02_plan/screen/problem.md` v0.4 SCR-PRB-003(전이 라벨표 89~94행).
- 공통 아키텍처: `docs/03_develop/plan/common.md` "상태 전이 버튼 라벨·타임라인 actor 공통 아키텍처" 절. **PROBLEM은 타임라인 actor 대상 아님**(BE/DB 변경 없음, FE 단독).

### 담당 범위 — dev-fe 단독(BE/DB 없음)
- `features/problem/status.ts`에 `transitionLabel(t, target: ProblemStatus): string` 신규(i18n 키 `problem:transition.*`, 매핑값 SCR-PRB-003 89~94행 표). 기존 `statusLabel`은 변경하지 않음.
- `ProblemDetailPage.tsx`의 전이 버튼 텍스트만 `statusLabel(t, target)` → `transitionLabel(t, target)`으로 교체(토스트 문구는 `statusLabel` 유지).

### 완료 기준
- 문제 상세(SCR-PRB-003)의 전이 버튼에 동작 동사형 라벨이 표시되고, 전이 완료 토스트는 기존처럼 도착 상태명을 사용한다.
