...
| SCR-SRM-009 | 서비스 카탈로그 카테고리 관리 | 카테고리 고정 목록 CRUD |
...
| API-SRM-019 | /api/v1/service-catalog/categories | POST | 카테고리 생성 |
| API-SRM-020 | /api/v1/service-catalog/categories/{id} | PATCH | 카테고리 수정 |
| API-SRM-021 | /api/v1/service-catalog/categories/{id} | DELETE | 카테고리 삭제(참조 중이면 409) |
...
- **카탈로그 카테고리 생성·수정·삭제(API-SRM-019/020/021)는 PROCESS_OWNER 전용**(그 외 → 403). 목록 조회(API-SRM-018)는 인증된 사용자 전체 공개.
