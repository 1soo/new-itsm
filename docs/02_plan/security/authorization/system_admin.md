...
> 역할: SYSTEM_ADMIN · 버전: 0.2 · 작성일: 2026-07-11 · 메뉴 관리(SCR-ADMIN-006, API-AUTH-016~021) 및 전 역할 공통 API-AUTH-022 반영
...
- 화면 이동마다 Backend에 권한 확인 API를 호출한다(admin 전용 라우트(SCR-ADMIN-*, API-AUTH-006~021)는 다른 역할이 접근하면 403, SYSTEM_ADMIN은 항상 허용).
