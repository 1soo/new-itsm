...
> 역할: CHANGE_MANAGER · 버전: 0.2 · 작성일: 2026-07-11 · 승인 프로세스 커스텀 기능(유지보수 요청) 반영 — 위험도 기반 CAB 자동 라우팅 제거에 따라 전용 승인 API 언급을 공통 승인 API(API-COM-005) 기준으로 갱신
...
- **승인 결정(API-COM-005, [common.md](../../02_plan/api_spec/common.md))은 해당 변경에 매칭된 승인 프로세스 규칙의 차수가 요구하는 역할 보유자 전용**이며, CHANGE_MANAGER가 그 역할에 지정되어 있지 않으면 단독으로는 승인 불가(승인 권한은 SYSTEM_ADMIN이 SCR-ADMIN-008에서 규칙으로 별도 부여). 승인 완료 전 구현 전이 시도는 409.
