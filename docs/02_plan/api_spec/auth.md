...
> 도메인: auth · 버전: 0.7 · 작성일: 2026-07-15 · 요청 유형별 담당자 역할 지정 기능(유지보수 요청) 반영 — 역할 목록 조회(공개) API(API-AUTH-030) 신규
>
> 이전 버전: 감사 로그 actor 오기록 결함 수정(유지보수 요청) — API 계약 변경 없음, actor 기록 주체를 현재 로그인 관리자로 정정(2절 참조) · 승인 프로세스 범위 우선순위 재설계(유지보수 요청) — API-AUTH-027/028 `domain` 선택 필드로 변경(미지정=전체 도메인), 우선순위 산정·충돌(409) 판정 로직 재정의(`database/common.md` 참조) · Role-Menu 동적 매핑(유지보수 요청) API 추가(API-AUTH-016~022, 기존 screen/screen_role 재사용) · 승인 프로세스 커스텀 기능(유지보수 요청) 관리자 CRUD API 추가(API-AUTH-023~029, SYSTEM_ADMIN 전용) · 토큰 저장 방식 Client Memory 전환 및 /auth/refresh CSRF 토큰 검증 추가(유지보수 요청) · 사이드바 메뉴 i18n 미적용 결함 수정(유지보수 요청) — API-AUTH-016/017/018/022에 영문 메뉴명/그룹명 필드 추가
...
- **감사 로그 actor 기록 규칙(2026-07-15 결함 수정)**: 관리자 CRUD API가 `audit_log`에 남기는 `actor_id`/`actor_email`은 **현재 로그인한 관리자**(요청을 수행한 Access Token의 principal)를 기준으로 기록한다. 변경 대상 리소스(계정 등)의 식별값은 `target` 컬럼에만 담는다(actor와 target을 혼동해 대상 리소스 자신을 actor로 기록하지 않는다).
...
- **Response Body** (200):
  ...
  ```
  > `screenNameEn`/`groupLabelEn`은 2026-07-13 유지보수 요청(사이드바 메뉴 i18n)으로 추가된 영문 필드다(`database/auth.md` 5절 `screen.screen_name_en`/`group_label_en`).
- **Response Code**: 200 정상 / 401 미인증 / 403 권한 부족
...
- **Response Body** (200):
  ...
  ```
  > `nav_visible=true`인 화면만 포함하며, `sort_order` 오름차순으로 정렬한다. 그룹 표시 순서는 그룹별 최소 `sort_order` 값 기준으로 정렬한다(`docs/02_plan/database/auth.md` 5절). `groupLabelEn`/`screenNameEn`은 2026-07-13 유지보수 요청(사이드바 메뉴 i18n)으로 추가됐다 — FE(`AppLayout.tsx`)가 `i18n.language`(`en`이면 `*En` 필드, 그 외 한국어 필드)에 따라 표시값을 선택한다.
- **Response Code**: 200(그룹 없으면 빈 배열) / 401 미인증

### API-AUTH-023 · 승인 프로세스 대상 도메인 목록 조회

승인 프로세스 생성 화면(SCR-ADMIN-008) "규칙 정보" 카드의 도메인 선택 선택지(2026-07-13 메타데이터 분리 이전에는 0단계). `hasRequestSubtype=true`인 도메인만 요청 유형 선택(구 1단계)을 노출한다. 반환 대상은 **요청자가 제출해 진행되는 티켓/요청 개념이 있는 9개 도메인**(SERVICE_REQUEST, CHANGE, KNOWLEDGE, INCIDENT, PROBLEM, ASSET, VULNERABILITY, COMPLIANCE, ESM)으로 한정한다. AUTH·COMMON·INFRA_MONITORING은 진행 중인 티켓/요청자 개념 자체가 없어 이 목록에 포함하지 않는다(maintainer 확인, 2026-07-11).
...
- **Response Body** (200):
  ...
  ```
- **Response Code**: 200 / 401 / 403
  > 이 목록은 여전히 9개 실제 도메인만 반환한다(2026-07-15 우선순위 재설계로 규칙 자체는 "도메인 미지정(전체)"을 지원하게 됐지만, 대상 도메인 후보 자체는 변경 없음). "전체 도메인" 선택지는 API 응답에 포함하지 않으며 SCR-ADMIN-008 생성 화면이 이 9개 목록 위에 클라이언트 전용 의사(pseudo) 옵션으로 추가한다(요청유형의 "전체" 옵션과 동일한 패턴, API-AUTH-024 참조). 생성 요청 시 이 의사 옵션을 선택하면 `domain: null`로 전송한다(API-AUTH-027).

### API-AUTH-024 · 도메인별 요청유형 후보 목록 조회

`hasRequestSubtype=true`인 도메인(SERVICE_REQUEST=카탈로그 항목, CHANGE=변경유형)의 "규칙 정보" 카드 요청 유형 선택지(2026-07-13 메타데이터 분리 이전에는 1단계). `hasRequestSubtype=false`인 도메인 호출 시 빈 배열을 반환한다(요청 유형 선택 자체가 생략되므로 화면에서는 호출하지 않음).
...
- **Response Body** (200):
  ```json
  {
    "content": [
      {
        "id": "number", "domain": "string|null · null=전체 도메인 적용(2026-07-15 우선순위 재설계)", "requestSubtypeKey": "string|null", "requestSubtypeLabel": "string|null · API-AUTH-024 후보 목록에서 resolve",
...
- **Response Body** (200):
  ```json
  {
    "id": "number", "domain": "string|null · null=전체 도메인", "requestSubtypeKey": "string|null", "name": "string", "description": "string|null",
...
> **범위 우선순위 재설계(2026-07-15 유지보수 요청)**: 도메인·요청유형·승인요청자 역할 3축을 각각 독립적으로 지정할 수 있으며, 축을 비우면(도메인/요청유형=null, 요청자 역할=빈 배열) 해당 축의 모든 값에 매칭되는 것으로 간주한다. 우선순위(`priorityTier`)는 "지정된 축 개수"가 많을수록 우선하고, 개수가 같으면 "역할 > 요청유형 > 도메인" 순으로 우선한다(산정식은 `database/common.md` approval_process 참조). 단, 요청유형은 도메인에 종속된 어휘이므로 **도메인을 비우면 요청유형도 반드시 비워야 한다**.
...
- **Request Body**:
  ```json
  {
    "domain": "string|null · 선택, null=전체 도메인 적용(2026-07-15 우선순위 재설계로 필수→선택 변경)", "requestSubtypeKey": "string|null · domain이 null이면 반드시 null. domain이 하위유형 있는 도메인일 때만 값 지정 가능, 전체 선택 시 null",
...
- **Response Code**:
  ...
  |------|------|
  | 201 | 생성 성공 |
  | 400 | domain 지정 시 9개 후보(API-AUTH-023) 외 값 / domain이 null인데 requestSubtypeKey가 값을 가짐 / step의 roleIds 비어 있음 / steps 10개 초과 / decisionMode 정의되지 않은 값 |
  | 403 | 권한 부족(System Admin 아님) |
  | 409 | 우선순위 충돌(2026-07-15 3축 일반화) — 산정된 `priorityTier` 버킷별 판정: tier=0(전체 미지정)에 이미 규칙이 있는데 3축 모두 비운 채 재생성 시도 / 같은 domain에 tier=11(도메인만 지정) 규칙이 이미 있는데 동일 domain·나머지 축 미지정으로 재생성 시도 / 같은 (domain, requestSubtypeKey)에 tier=23(도메인+요청유형) 규칙이 이미 있는데 동일 스코프·역할 미지정으로 재생성 시도 / tier=14(역할만)·25(도메인+역할)·37(도메인+요청유형+역할)에 해당하는 동일 매칭 스코프(tier=14는 전체, tier=25는 동일 domain, tier=37은 동일 domain+requestSubtypeKey) 내 기존 규칙과 `requesterRoleIds` 조합이 하나라도 겹침(Q3 확정 답변 기준 일반화) |
...
- **Response Code**: 200 / 400 / 403 / 404 / 409(API-AUTH-027과 동일한 우선순위 충돌 검증 — domain·requestSubtypeKey는 기존 값 그대로 유지된 채 requesterRoleIds만 바뀌므로 tier 재산정 후 동일 스코프 내 겹침만 비교, 본인 규칙 제외)
...
- **Response Body** (200):
  ...
  ```
  > 2026-07-15 유지보수 요청(요청 유형별 담당자 역할 지정)으로 신규 추가. `/api/v1/admin/**`가 아니라 `PROCESS_OWNER` 등 비관리자 화면에서도 역할 선택 UI(예: 서비스 카탈로그 관리 SCR-SRM-007의 담당자 역할 select)가 호출할 수 있도록 별도 경로로 둔다. `userCount` 등 관리자 전용 필드는 노출하지 않는다.
