...
- **Response Body** (200):
  ```json
  {
    "content": [
      {
        "id": "number", "domain": "string|null · null=전체 도메인 적용", "requestSubtypeKey": "string|null", "requestSubtypeLabel": "string|null · API-AUTH-024 후보 목록에서 resolve",
...
> **범위 우선순위 규칙**: 도메인·요청유형·승인요청자 역할 3축을 각각 독립적으로 지정할 수 있으며, 축을 비우면(도메인/요청유형=null, 요청자 역할=빈 배열) 해당 축의 모든 값에 매칭되는 것으로 간주한다. 우선순위(`priorityTier`)는 "지정된 축 개수"가 많을수록 우선하고, 개수가 같으면 "역할 > 요청유형 > 도메인" 순으로 우선한다(산정식은 `database/common.md` approval_process 참조). 단, 요청유형은 도메인에 종속된 어휘이므로 **도메인을 비우면 요청유형도 반드시 비워야 한다**.
...
- **Request Body**:
  ```json
  {
    "domain": "string|null · 선택, null=전체 도메인 적용", "requestSubtypeKey": "string|null · domain이 null이면 반드시 null. domain이 하위유형 있는 도메인일 때만 값 지정 가능, 전체 선택 시 null",
...
- **Response Code**:
  ...
  | 409 | 우선순위 충돌 — 산정된 `priorityTier` 버킷별 판정: tier=0(전체 미지정)에 이미 규칙이 있는데 3축 모두 비운 채 재생성 시도 / 같은 domain에 tier=11(도메인만 지정) 규칙이 이미 있는데 동일 domain·나머지 축 미지정으로 재생성 시도 / 같은 (domain, requestSubtypeKey)에 tier=23(도메인+요청유형) 규칙이 이미 있는데 동일 스코프·역할 미지정으로 재생성 시도 / tier=14(역할만)·25(도메인+역할)·37(도메인+요청유형+역할)에 해당하는 동일 매칭 스코프(tier=14는 전체, tier=25는 동일 domain, tier=37은 동일 domain+requestSubtypeKey) 내 기존 규칙과 `requesterRoleIds` 조합이 하나라도 겹침 |
