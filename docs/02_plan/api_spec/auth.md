...
> 도메인: auth · 버전: 0.6 · 작성일: 2026-07-15 · 감사 로그 actor 오기록 결함 수정(유지보수 요청) — API 계약 변경 없음, actor 기록 주체를 현재 로그인 관리자로 정정(2절 참조) · 승인 프로세스 범위 우선순위 재설계(유지보수 요청) — API-AUTH-027/028 `domain` 선택 필드로 변경(미지정=전체 도메인), 우선순위 산정·충돌(409) 판정 로직 재정의(`database/common.md` 참조)
>
> 이전 버전: Role-Menu 동적 매핑(유지보수 요청) API 추가(API-AUTH-016~022, 기존 screen/screen_role 재사용) · 승인 프로세스 커스텀 기능(유지보수 요청) 관리자 CRUD API 추가(API-AUTH-023~029, SYSTEM_ADMIN 전용) · 토큰 저장 방식 Client Memory 전환 및 /auth/refresh CSRF 토큰 검증 추가(유지보수 요청) · 사이드바 메뉴 i18n 미적용 결함 수정(유지보수 요청) — API-AUTH-016/017/018/022에 영문 메뉴명/그룹명 필드 추가
