...
| form_schema | JSONB | NOT NULL, DEFAULT `{"components":[]}` | 동적 양식 스키마(자체 8×n 그리드 — `components` 배열, 각 컴포넌트는 `type`(text/textarea/select/radio/checkbox/date/file/label/guide)·`key`·`position`{col,row}·`size`{w,h}·Content 설정을 포함. 7개 입력 타입은 `input.placeholder` 필드를 갖는다. `label` 타입은 표시 텍스트·정렬만 가지며, `guide` 타입은 안내 텍스트·첨부 가이드 파일(base64 데이터 URL 인라인)만 가진다 — 둘 다 값 입력이 없어 제출 데이터(`service_request.form_values`) 대상에서 제외된다. 상세 스키마는 [api_spec/service-request.md](../api_spec/service-request.md) API-SRM-002 참조). SCR-SRM-007 "Form 설정" 팝업(자체 그리드 빌더, [screen/service-request.md](../screen/service-request.md) 5절)이 편집·저장 |
| ...공통 컬럼... | | | |
