...
| approval_process | 승인 프로세스 정의(규칙 헤더, 전 도메인 공용) | 유지보수 요청(2026-07-11, 승인 프로세스 커스텀) |
...
| notification_dismissal | 사용자별 알림 확인처리(dismiss) 이력 | Main 요청(2026-07-11, 헤더 알림 확인처리) |
...
규칙의 승인요청자 역할 스코프(승인 요청자 박스에서 선택한 역할). 0개면 요청자 무관(도메인/요청유형 tier), 1개 이상이면 그중 하나라도 보유한 사용자가 요청한 티켓에 매칭(ANY, Q2 확정 답변 — AND/OR 개념 없음).
