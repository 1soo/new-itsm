...
- **목적**: 통합 검색·알림·테마 전환·계정 접근 진입점.
- **레이아웃**: 좌측 로고·현재 도메인 타이틀 / 중앙 통합 검색바 / 우측 테마 토글·알림 벨·사용자 아바타 메뉴.
- **구성 요소**:
  | 요소 | 유형 | 설명 | 색상 |
  |------|------|------|------|
  | 통합 검색바 | 입력 | 지식/티켓 키워드 검색 진입 | `--border`/`--ring`(포커스) |
...
- **상태 · 인터랙션**: 사용자 메뉴 > 로그아웃 클릭 시 확인 후 토큰 무효화. 알림 없으면 뱃지 숨김. 테마 토글 클릭 시 SCR-COM-010 동작 수행.
