...
> 도메인: common · 버전: 0.5 · 작성일: 2026-07-11 · 사용자 가이드 모달(REQ-COM-002/FEAT-COM-002) 반영, 알림 드롭다운 2줄 레이아웃(도메인·상대시간/제목, REQ-COM-001/FEAT-COM-001) 반영, UI/UX 개편(ADS 기반, REQ-UIX-001~013) 반영
...
| SCR-COM-012 | 사용자 가이드 | REQ-COM-002 | 헤더 "?" 아이콘 클릭 시 개요·도메인 및 원칙·역할별 수행 내용과 방법을 보여주는 모달 |
...
- **구성 요소**:
  ...
  | 사용자 가이드 아이콘 | 아이콘 버튼("?") | 클릭 시 사용자 가이드 모달 오픈(SCR-COM-012), 테마 토글 왼쪽 배치 | `--foreground`, `aria-label`="사용자 가이드" |
...
- **목적**: 인증된 사용자가 플랫폼 개요·업무 도메인별 목적과 원칙·역할별 수행 내용과 방법을 한 곳에서 확인할 수 있게 한다(REQ-COM-002/FEAT-COM-002).
- **레이아웃**: 대형 모달(SCR-COM-009 모달 패턴 재사용, Overlay elevation, `radius.xlarge`). 상단 타이틀 "사용자 가이드"+닫기(X) 버튼 / 그 아래 3개 탭("개요"·"도메인 및 원칙"·"역할별 수행 내용과 방법") / 탭별 콘텐츠 영역(세로 스크롤).
- **구성 요소**:
  | 요소 | 유형 | 설명 | 색상 |
  |------|------|------|------|
  | 탭 네비게이션 | 탭 | 개요/도메인 및 원칙/역할별 수행 내용과 방법 전환 | `--primary`(활성 탭) |
  | 개요 콘텐츠 | 텍스트 블록 | 플랫폼 전체 소개 1개 문단(`docs/01_analyze/feature/user-guide-content.md` 1절 근거) | `--foreground` |
  | 도메인 아코디언 | 아코디언 리스트 | 11개 업무 도메인(auth·service-request·incident·problem·change·knowledge·asset·esm·vulnerability·compliance·infra-monitoring) 각각 도메인명+목적+핵심 원칙(동 문서 2절 근거), 역할 무관 전체 노출 | `--foreground`/`--card` |
  | 역할 아코디언 | 아코디언 리스트 | 정의된 16개 역할 각각 역할명+페르소나+주요 수행 내용과 방법(동 문서 3절 근거) | `--foreground`/`--card` |
  | 내 역할 배지 | Lozenge | 로그인 사용자가 보유한 역할에 표시, Info tone | Info(subtle) |
  | 닫기 버튼 | 아이콘 버튼 | 모달 닫기 | `--foreground`, `aria-label`="닫기" |
- **상태 · 인터랙션**:
  - 헤더 "?" 아이콘 클릭 시 모달 오픈(기본 탭 = "개요"). 배경 클릭/Esc/닫기 버튼으로 닫힘(SCR-COM-009 모달 모션 2.6절 적용).
  - "도메인 및 원칙" 탭: 11개 도메인 아코디언은 로그인 사용자의 역할과 무관하게 항상 전체 노출(시스템 전체 소개 목적, 필터링 없음). 기본은 모두 접힌 상태이며 클릭 시 개별 펼침.
  - "역할별 수행 내용과 방법" 탭: 로그인 사용자가 보유한 역할(1개 이상)은 목록 최상단에 "내 역할" 배지와 함께 기본 펼쳐진 상태로 고정 노출하고, 나머지 역할은 그 아래 접힌 아코디언으로 나열한다(클릭 시 펼침). 사용자가 역할을 보유하지 않은 경우(이론상 없음)에도 16개 역할 전체가 접힌 상태로 노출되어 탐색 가능해야 한다.
  - 콘텐츠는 정적 데이터로 제공되며 탭 전환 시 서버 재조회는 없다.
- **연관 API**: 없음(정적 콘텐츠, `docs/01_analyze/feature/user-guide-content.md` 기준 FE 정적 데이터)
