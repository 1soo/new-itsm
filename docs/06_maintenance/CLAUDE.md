...
| 20260719-001132 | srm | 그리드 폼 빌더 미니팝업 위치·순서 보정(7차): Content 설정 팝업 라벨 지정 Select를 최상단으로 이동, Content 설정 팝업(Popover→Modal 전환)과 라벨 생성/수정 팝업 위치를 MINI_POPUP_POSITION_CLASS 공유로 통일(가로 정중앙·세로 정중앙보다 살짝 위, 공용 ui/dialog.tsx는 미변경) | docs/06_maintenance/20260719-001132/srm/history.md |
| 20260719-082912 | srm | 그리드 폼 빌더 세부 개선 8차: 라벨(태그) 경계 그룹 표시를 캔버스 전용에서 dynamic-form-renderer.tsx로 확대(요청 제출 폼·A1 축소 미리보기 모두 노출, GridLabelOverlays 공유), date/file/select 하드코딩 기본 placeholder 완전 제거, Content 설정 기본값 UI를 실제 입력 타입과 동일화(select/radio 단일 pill·checkbox 다중·date input·file UI 제거, defaultValue string\|string[]\|null 확장), 읽기전용/필수여부 체크박스를 팝업 타이틀 우측으로 이동(공용 Modal titleExtra 신규), form io 파일 i18n 전환 | docs/06_maintenance/20260719-082912/srm/history.md |
