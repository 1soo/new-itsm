...
|------|------|
| 2026-07-10 | 최초 작성 |
| 2026-07-10 | raw 색상 하드코딩 화면 재검증: 최초 grep 오탐 확인, 해당 없음으로 정정 |
...
- **출력**: 개별 화면 조정 필요 목록 — **해당 없음**(4절 도메인 영향 범위 참고)
- **처리 흐름**:
...
토큰·공통 컴포넌트 개편은 모든 화면에 **자동 반영**된다(각 도메인 화면은 `components/ui`, `components/common`, `components/layout`을 조합해 구성되어 있기 때문). raw Tailwind 색상 클래스가 하드코딩된 화면은 없다.

| 도메인 | 영향 유형 | 근거 |
|--------|-----------|------|
| auth | 자동 반영 | raw 색상 하드코딩 미발견 |
| auth(관리자 화면) | 자동 반영 | raw 색상 하드코딩 미발견 |
| service-request | 자동 반영 | raw 색상 하드코딩 미발견 |
| incident | 자동 반영 | raw 색상 하드코딩 미발견 |
