...
전 도메인(인시던트/문제/서비스요청) 티켓이 공유하는 공통 요소. 댓글·타임라인·티켓 연계 모델과 티켓 공통 enum.
승인은 2026-07-11 승인 프로세스 커스텀 기능으로 `common.approval`(전 도메인 공용 다차 승인 엔진)로 완전히 이전되었다(이 패키지의 `Approval`/`ApprovalStatus`/`ApprovalRepository`는 제거됨).
