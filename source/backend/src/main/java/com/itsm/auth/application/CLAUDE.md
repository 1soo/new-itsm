...
- `ApprovalProcessAdminService.java` — 승인 프로세스 정의 CRUD 유스케이스(API-AUTH-023~029/031, SYSTEM_ADMIN 전용, 2026-07-11 승인 프로세스 커스텀 기능, 2026-07-22 범위 우선순위 4축 재설계). 대상 도메인 9개 하드코딩(domain은 선택, null=전체 도메인), 요청유형 후보는 CHANGE 고정 코드+`common.approval.application.ApprovalRequestSubtypeProvider` 구현 빈(SERVICE_REQUEST 등) 위임. `states(domain)`(API-AUTH-031)은 `common.approval.application.TargetStateLabelResolver.statesOf()`를 그대로 재사용(중복 구현 없음). priorityTier는 domain/targetState/requestSubtypeKey/requesterRoleIds 4축 독립 스코프 기반 산정식(축개수×10+역할4+요청유형2+적용상태8+도메인1, 실제값 0/11/14/23/25/37/43/55)이며 domain이 null이면 requestSubtypeKey·targetState도 null 강제. targetState 지정 시 requesterRoleIds 최소 1개 필수(`validateTargetStateRequiresRole`) + `states(domain)` 후보값 중 하나여야 함(`validateTargetStateIsValidOption`). tier=0/11/23은 존재 검증, tier=14/25/37/43/55는 동일 매칭 스코프(43/55는 domain+targetState[+subtype])내 역할 조합 교집합 검증(409). 실제 게이트 체크·인스턴스 조회/결정은 `common.approval` 담당(이 서비스는 규칙 정의만)
