...
- `Screen.java` — 화면 엔티티(BaseEntity 상속). screenCode/screenName/screenNameEn/path/domain + 사이드바 표시 컬럼(iconName/groupCode/groupLabel/groupLabelEn/sortOrder/navVisible, Role-Menu 동적 매핑). screenNameEn/groupLabelEn은 2026-07-13 사이드바 메뉴 i18n 유지보수로 추가(`database/auth.md` 5절). `update()`로 부분 수정(null 필드는 미변경) 지원
