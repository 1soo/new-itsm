...
- `ServiceCatalogService.java` — 서비스 카탈로그 항목 CRUD·동적 양식 관리(승인 필요 여부 필드는 제거됨). 담당자 역할(assigneeRoleId, 2026-07-15) CRUD 지원, 상세 응답에 `RoleRepository`로 역할명(assigneeRoleName) resolve해 포함. `category`(자유 텍스트) → `categoryId`(FK) 전환(2026-07-16 유지보수 요청) — 생성/수정 시 `ServiceCatalogCategoryRepository`로 존재 검증(404 CATEGORY_NOT_FOUND), 목록/상세 응답에 `categoryName` resolve해 포함
