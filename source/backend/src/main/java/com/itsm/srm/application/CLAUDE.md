...
## 파일
- `ServiceRequestService.java` — 요청 생성·상태전이·배정·댓글·CSAT·지식연계·자산연계 조회(REQ-ITAM-006, TicketLink 재사용) 유스케이스. IN_FULFILLMENT 전이 시 공용 승인 게이트(`common.approval.application.ApprovalGateService`) 호출(2026-07-11 승인 프로세스 커스텀 기능, 승인 결정·대기 목록은 이 서비스가 아닌 공용 엔진이 담당)
- `ServiceCatalogService.java` — 서비스 카탈로그 항목 CRUD·동적 양식 관리(승인 필요 여부·담당 역할 필드 제거됨)
