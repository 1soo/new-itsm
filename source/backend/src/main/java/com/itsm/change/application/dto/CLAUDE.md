...
change 도메인 애플리케이션 계층의 요청·응답 DTO(record/enum). 승인 관련 DTO(ChangeApprovalDecision/Request/Response, PendingChangeApprovalResponse)는
...
- `ChangeDetailResponse.java` — 상세 응답(구현결과·승인(approvalRequestId/status, common.approval 조회)·연계·허용 상태전이 포함)
