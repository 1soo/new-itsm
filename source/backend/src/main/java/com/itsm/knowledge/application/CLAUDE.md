...
## 파일
- `KnowledgeService.java` — 지식 기사 검색/목록·상세·작성/수정/삭제·상태전이(공용 승인 게이트)·유용성평가·카테고리·KCS 티켓 연계·지표 유스케이스. 검토요청(API-KM-006)은 `ApprovalGateService.evaluateAndCreateIfNeeded()`로 항상 200 성공, 매칭 여부로 즉시 PUBLISHED/IN_REVIEW 분기(구 검토승인/반려/검토대기 코드 2026-07-11 제거)
