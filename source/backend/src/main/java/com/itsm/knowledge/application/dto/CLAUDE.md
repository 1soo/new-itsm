...
- `ArticleSummaryResponse.java` — 목록 요약 응답(요약·상태·카테고리·유용성 비율)
- `ArticleListResponse.java` — 검색/목록 응답(페이지 정보 + 무결과 검색 여부)
- `ArticleDetailResponse.java` — 상세/열람 응답(본문·라벨·유용성 집계)
- `StatusTransitionRequest.java` — 상태 전이 요청(targetStatus)
- `StatusResponse.java` — 상태 응답(id, status)
- `ReviewRequest.java` — 검토·게시 승인/반려 요청(decision, reason)
- `ReviewResponse.java` — 검토 응답(id, status)
- `PendingReviewResponse.java` — 검토 대기 목록 응답
