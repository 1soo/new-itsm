...
> `KnowledgeReview`/`ReviewDecision`(검토·게시 승인/반려 이력)은 승인 프로세스 커스텀 기능(2026-07-11)으로 제거되었다 — `knowledge_review` DB 테이블은 유지되지만 대응 JPA 엔티티는 없다(공용 승인 엔진 `common.approval`의 `approval_decision`이 대체).
