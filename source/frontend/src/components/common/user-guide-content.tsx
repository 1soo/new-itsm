 ...
 * `docs/01_analyze/feature/user-guide-content.md`(v0.2)를 가공 없이 그대로 옮긴 정적 데이터를 렌더링한다.
 * 페이지 레이아웃(문서 헤더·TOC·섹션 배치)은 FE(`features/guide/GuidePage.tsx`)가 담당한다.
 */
...
const OVERVIEW_MARKDOWN = `이 ITSM(IT Service Management) 플랫폼은 한 조직의 IT 운영과 사내 서비스 지원 업무 전체를 하나의 시스템 안에서 처리할 수 있도록 설계되었습니다.
...
export function UserGuideOverview({ className }: { className?: string }) {
  return <Markdown className={className}>{OVERVIEW_MARKDOWN}</Markdown>;
}
        ...
        <AccordionItem key={r.code} value={r.code}>
          <AccordionTrigger>
            <span className="flex items-center gap-2">
              {r.name}
              {myRoleSet.has(r.code) ? <StatusBadge tone="info" label="내 역할" /> : null}
            </span>
          </AccordionTrigger>
          <AccordionContent>
