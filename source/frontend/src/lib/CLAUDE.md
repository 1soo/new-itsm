...
공통 유틸과 HTTP 클라이언트. 모든 API 요청은 여기 apiClient 경유(직접 fetch/axios 금지).
...
- `icon.ts` — `resolveIcon(iconName)`: 문자열(lucide-react 컴포넌트명) → 실제 아이콘 컴포넌트. 없으면 기본 아이콘(HelpCircle)로 폴백. 사이드바(`routes/AppLayout.tsx`)와 메뉴 관리(`features/admin/MenuManagementPage.tsx`)가 공유.
