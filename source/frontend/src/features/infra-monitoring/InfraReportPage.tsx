...
export function InfraReportPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold text-foreground">인프라 지표 리포팅</h1>
      ...
      <form onSubmit={handleSearch} className="flex flex-wrap items-end gap-2 rounded-lg border border-border bg-card p-3">
        ...
        <div className="space-y-1">
          <Label htmlFor="assetId">자산 ID (선택)</Label>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <KpiCard label="평균 가동률" value={loading ? "-" : Math.round(report?.avgUptime ?? 0)} unit="%" />
        <KpiCard label="평균 CPU" value={loading ? "-" : Math.round(report?.avgCpu ?? 0)} unit="%" />
        <KpiCard label="평균 메모리" value={loading ? "-" : Math.round(report?.avgMemory ?? 0)} unit="%" />
        <KpiCard label="평균 응답시간" value={loading ? "-" : Math.round(report?.avgResponseTime ?? 0)} unit="ms" />
        <KpiCard label="평균 용량 활용률" value={loading ? "-" : Math.round((report?.avgCapacityUtilization ?? 0) * 100)} unit="%" />
      </div>
    </div>
  );
}
