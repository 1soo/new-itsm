...
서비스 요청(SRM) 관리 기능. 포털·요청 제출·목록·큐·상세·카탈로그 관리·지표 화면과 API·타입, 상태/SLA 표시 매핑을 제공한다. END_USER/SERVICE_DESK_AGENT/PROCESS_OWNER 역할 기반. API 계약은 service-request.md 기준. 승인 대기·결정은 승인 프로세스 커스텀 기능(유지보수 요청)으로 `features/common/ApprovalInboxPage.tsx`(SCR-COM-014, 전 도메인 공용)로 이관됐다(기존 SCR-SRM-006 제거).
...
- `RequestDetailPage.tsx` — 요청 상세·상태 전이·코멘트·CSAT(SCR-SRM-005). 승인 패널은 공용 `ApprovalPanel`(`components/common`)에 API-COM-004 조회 결과(`steps`/`currentStepNo`)를 주입해 진행 상태만 표시(결정 액션 없음, 처리는 SCR-COM-014에서).
