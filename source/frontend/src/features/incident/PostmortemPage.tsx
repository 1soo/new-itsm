...
export function PostmortemPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError(null);
    if (!rootCause.trim()) {
      setError(t("postmortem.rootCauseRequiredError", { defaultValue: "근본 원인은 필수입니다." }));
      return;
    }
    ...
    try {
      ...
      toast.success(t("postmortem.saveSuccess", { defaultValue: "포스트모템이 저장되었습니다" }));
      navigate(`/incidents/${id}`);
    } catch (err) {
      setError(extractErrorMessage(err, t("postmortem.saveFailed", { defaultValue: "저장에 실패했습니다." })));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="mx-auto max-w-2xl space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-foreground">
          {t("postmortem.title", { defaultValue: "포스트모템" })}
        </h1>
        <Button variant="outline" onClick={() => navigate(`/incidents/${id}`)}>
          {t("postmortem.backToIncident", { defaultValue: "인시던트로" })}
        </Button>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        <Card>
          <CardHeader><CardTitle className="text-base">{t("postmortem.overviewTitle", { defaultValue: "개요" })}</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="s">{t("postmortem.summary", { defaultValue: "요약" })}</Label>
              <Input id="s" value={summary} onChange={(e) => setSummary(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="tl">{t("postmortem.timelineSummary", { defaultValue: "타임라인 요약" })}</Label>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {fiveWhys.map((w, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <Input
                  ...
                  placeholder={t("postmortem.whyPlaceholder", { index: i + 1, defaultValue: `왜? ${i + 1}` })}
                />
                <Button
                  type="button"
                  variant="ghost"
                  size="icon"
                  aria-label={t("postmortem.delete", { defaultValue: "삭제" })}
                  onClick={() => removeWhy(i)}
                >
            ...
            <Button type="button" variant="outline" size="sm" onClick={addWhy}>
              <Plus /> {t("postmortem.addWhy", { defaultValue: "Why 추가" })}
            </Button>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="text-base">{t("postmortem.rootCauseTitle", { defaultValue: "근본 원인" })}</CardTitle></CardHeader>
          <CardContent className="space-y-1.5">
            <Input
              value={rootCause}
              onChange={(e) => setRootCause(e.target.value)}
              aria-invalid={!!error && !rootCause.trim()}
              required
              placeholder={t("postmortem.rootCausePlaceholder", { defaultValue: "근본 원인(필수)" })}
            />
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="text-base">{t("postmortem.actionItemsTitle", { defaultValue: "조치 항목" })}</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {actionItems.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                {t("postmortem.noActionItems", { defaultValue: "조치 항목이 없습니다." })}
              </p>
            ) : (
              actionItems.map((a, i) => (
                <div key={i} className="grid gap-2 rounded-md border border-border p-3 sm:grid-cols-[1fr_8rem_9rem_7rem_auto]">
                  <Input
                    ...
                    placeholder={t("postmortem.actionDescription", { defaultValue: "조치 내용" })}
                  />
                  <Input
                    value={a.owner}
                    onChange={(e) => setAction(i, { owner: e.target.value })}
                    placeholder={t("postmortem.actionOwner", { defaultValue: "담당" })}
                  />
                  ...
                  <Select value={a.status} onValueChange={(v) => setAction(i, { status: v as ActionItem["status"] })}>
                    ...
                    <SelectContent>
                      <SelectItem value="OPEN">{t("postmortem.actionStatusOpen", { defaultValue: "진행" })}</SelectItem>
                      <SelectItem value="DONE">{t("postmortem.actionStatusDone", { defaultValue: "완료" })}</SelectItem>
                    </SelectContent>
                  </Select>
                  <Button
                    type="button"
            ...
            <Button type="button" variant="outline" size="sm" onClick={addAction}>
              <Plus /> {t("postmortem.addActionItem", { defaultValue: "조치 항목 추가" })}
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={() => navigate(`/incidents/${id}`)}>
            {t("postmortem.cancel", { defaultValue: "취소" })}
          </Button>
          <Button type="submit" loading={saving}>
            {t("postmortem.save", { defaultValue: "저장" })}
          </Button>
        </div>
      </form>
    </div>
