...
- `ProblemDetailPage.tsx` — 문제 상세·상태 전이(RESOLVED_CLOSED 전이 승인 게이트로 버튼 disable+tooltip)·RCA·조치(SCR-PRB-003). 승인 패널은 공용 `ApprovalPanel`(`components/common`)에 API-COM-004 조회 결과 주입해 진행 상태만 표시(매칭 없으면 패널 미노출).
