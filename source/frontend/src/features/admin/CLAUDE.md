...
- `ApprovalProcessFormPage.tsx` — 승인 프로세스 생성/편집(SCR-ADMIN-008). 0~3단계 카드 스택·역할 선택 슬라이드 패널·드래그 재정렬·박스별 필수역할 검증·승인자 0개 확인 다이얼로그는 공용 `ApprovalProcessFlow`(`components/common`)가 담당하며, 이 화면은 도메인/요청유형/이름 상태와 API 연동(도메인·역할 후보 조회, 생성/수정 payload 조립: `requester.roleIds`/`approvers[].{roleIds,matchType}`는 string ID → number 변환)만 조립한다. 편집 시 domain·requestSubtypeKey는 식별 스코프라 `domainDisabled`/`requestSubtypeDisabled` prop으로 선택을 비활성화한다(API-AUTH-028).
