...
export function AssetMetricsPage() {
  ...
  const dist: DistributionDatum[] = metrics
    ? [
        { label: typeLabel("HARDWARE"), value: metrics.typeDistribution?.HARDWARE ?? 0, tone: "info" },
        { label: typeLabel("SOFTWARE"), value: metrics.typeDistribution?.SOFTWARE ?? 0, tone: "muted" },
        { label: typeLabel("CLOUD"), value: metrics.typeDistribution?.CLOUD ?? 0, tone: "info" },
      ]
    : [];

  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold text-foreground">자산 지표</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <KpiCard label="활용률" value={loading ? "-" : Math.round(metrics?.utilizationRate ?? 0)} unit="%" />
        <KpiCard label="만료 임박" value={loading ? "-" : (metrics?.expiringCount ?? 0)} unit="건" />
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <DistributionChart data={dist} ariaLabel="자산 유형별 분포" />
        </CardContent>
      </Card>
    </div>
  );
