...
- `types.ts` — CHG 도메인 타입(6단계 `ChangeStatus`, `ChangeType`/`Risk`, `ChangeSummary`/`ChangeDetail`/`ChangeMetrics` 등). `ChangeDetail.approval`(`ChangeApproval`)은 `approvalRequestId`/`status`/`targetState`(2026-07-22 신규, 원본 코드값)를 보유(진행 상태 조회는 공용 API-COM-004), `ChangeSummary.pendingApprovalTargetState`(2026-07-22 신규)도 포함. `ApprovalRoute`/`ChangeApprovalRecord`/`ApprovalQueueItem`은 제거됨.
...
- `ChangeListPage.tsx` — 변경 목록(SCR-CHG-001). 상태 배지는 `pendingApprovalTargetState` 존재 시 공용 `deriveApprovalStatusDisplay`로 파생 표시(2026-07-22 신규).
- `ChangeCreatePage.tsx` — 변경 요청(RFC) 생성(SCR-CHG-002). 표준 변경 선택 시 템플릿 조회·승인 생략 안내.
- `ChangeDetailPage.tsx` — 변경 상세·상태 전이(승인 전 구현 전이 UI 차단)·구현결과·연계(SCR-CHG-003). 승인 패널은 공용 `ApprovalPanel`(`components/common`)에 API-COM-004 조회 결과 주입해 진행 상태만 표시. 상단 상태 배지는 `deriveApprovalStatusDisplay`로 파생 표시, `ApprovalPanel`에 `targetStateLabel`/`status`/`onResubmit`(반려 시 API-COM-006 재승인요청) 배선(2026-07-22 신규).
