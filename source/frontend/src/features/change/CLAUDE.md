...
변경(CHG) 관리 기능. 목록·RFC 생성·상세(6단계 전이·구현결과·연계)·일정 캘린더·지표 화면과 API·타입, 상태/유형/위험도 표시 매핑을 제공한다. CHANGE_MANAGER/APPROVER 역할 기반. API 계약은 change.md 기준. 승인 대기·결정은 승인 프로세스 커스텀 기능(유지보수 요청)으로 `features/common/ApprovalInboxPage.tsx`(SCR-COM-014, 전 도메인 공용)로 완전히 이관됐다(기존 SCR-CHG-004 제거, 위험도 기반 CAB 자동 라우팅·`approvalRoute` 개념도 함께 제거 — 실제 게이트 차단(409)은 Stage 2에서 연동, 이번 스테이지는 화면·데이터 형태만 최종 스펙으로 전환).
...
- `ChangeDetailPage.tsx` — 변경 상세·상태 전이(승인 전 구현 전이 UI 차단)·구현결과·연계(SCR-CHG-003). 승인 패널은 공용 `ApprovalPanel`(`components/common`)에 API-COM-004 조회 결과를 주입해 진행 상태만 표시.
