...
export function KnowledgeMetricsPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold text-foreground">지식 지표</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="사용량" value={loading ? "-" : (metrics?.usageCount ?? 0)} unit="건" />
        <KpiCard label="무결과 검색" value={loading ? "-" : (metrics?.noResultSearchCount ?? 0)} unit="건" />
        <KpiCard label="유용성" value={loading ? "-" : Math.round(metrics?.helpfulRate ?? 0)} unit="%" />
        <KpiCard label="티켓 차단율" value={loading ? "-" : Math.round(metrics?.deflectionRate ?? 0)} unit="%" />
      </div>

      <Card>
        <CardHeader><CardTitle className="text-base">무결과 검색 키워드</CardTitle></CardHeader>
        <CardContent>
